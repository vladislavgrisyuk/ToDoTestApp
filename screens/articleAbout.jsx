...
const ArticleTabFooter = ({ article, moveDetailsView }) =>
{
    ...
    const tabLabels =
    {
        ...
        list: (focused) => (
            <Entypo name="list" size={ 23 } color={ focused ? globalVars.varColorBlack : globalVars.varColorGray } />
        ),
        comments: (focused) => (
    ...
    return (
        <Tab.Navigator style={ {
            ...
            <Tab.Screen options={ {
                ...
                { (props) => <InfoTabData  { ...props } article={ article } moveDetailsView={ moveDetailsView } /> }
            </Tab.Screen>
            <Tab.Screen options={ {
...
const ArticleAbout = ({ route }) =>
{
    const [yOffset, setYOffset] = useState(0);
    const [value, setValue] = useState({});
    React.useEffect(() =>
    {
        getArticleData(route.params.href).then(
            v =>
            {
                setValue(v)
                console.log('again')
    ...
    function moveDetailsView(val)
    {
        if (val == 'show')
            marginOffset.value = -300
        if (val == 'hide')
            marginOffset.value = DEFAULT_MARGIN_TOP
    }
    let touchY = 0
    return (
        <View style={ {
            ...
            >
                <View style={ {
                    ...
                    <ImageBackground style={ { width: '100%', height: '100%' } } blurRadius={ 8 } resizeMode='cover' source={ { uri: value.img } }>
                        <View style={ {
                            ...
                            <View style={ { borderRadius: 6, overflow: 'hidden', width: 220, height: 297, overflow: 'hidden', marginBottom: -120 } }>
                                <Image source={ { uri: value.img } } style={ {
                ...
                <Animated.View
                    ...
                    // } }
                    style={ [{ borderRadius: 18, zIndex: 999, backgroundColor: 'white', flex: 1, marginTop: -30 }, myStyle] }>
                    <View>
                    ...
                    <View style={ {
                        ...
                        <Text style={ [styles.articleMetaDataText, styles.articleMetaDataPadding] }>{ value.year ?? '...' } </Text>
                        <Text style={ [styles.articleMetaDataText, styles.articleMetaDataPadding] }>{ value.type }</Text>
                        <View style={ [{
                            display: 'flex',
                            flexDirection: 'row',
                            ...
                            <Text style={ [{
                                ...
                                fontFamily: exo500,
                                color: '#868e96',
                            }, styles.articleMetaDataPadding] }>{ value.rating ?? '...' }</Text>
                            <Text style={ {
                                fontFamily: exo400,
                                color: '#868e96',
                                fontSize: 11,
                    ...
                    <ArticleTabFooter article={ value } moveDetailsView={ moveDetailsView } />
                </Animated.View>
            </View >
            <View style={ {
                width: 30,
